...
eval {
    ...
    $Start_Id=$Missing_Object_Ids[0];
    $End_Id=$Missing_Object_Ids[$#Missing_Object_Ids];

    my $batch_count = 5000;
    my $pages = ceil($End_Id/$batch_count);
    print "Paging through $pages of $batch_count objects\n";

    print "E: Workspace missing ".scalar(@Missing_Object_Ids)." objects\n";
	...
	open(DEL, ">> ${data}/NCBI_Deleted_Taxa.txt");
		...
		@temp=split(/\//,$ws_taxon->{'ws_ref'});
                print OUT $ws_taxon->{'taxon'}{'taxonomy_id'}."\t".$temp[1]."\n";
	    }
        }
	close(OUT);
	close(DEL);
	...
	close(OUT);
        ...
        if(scalar(@$saved_objects)){
            try{
                $impl->index_taxa_in_solr({taxa=>$Taxa_To_Load_in_SOLR,solr_core=>"taxonomy_prod",create_report=>0});
            }catch{
	...
	#Update Start ID for next block, instead of iterating through dozens of blocks of ignored objects
	$Start_Id = (grep { $_ > $end } @Missing_Object_Ids)[0];
    }
};
print $@,"\n" if $@;
